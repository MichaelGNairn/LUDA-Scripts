#### Michael Nairn Subnational Statistics 16 to 64 employment rate R Script ####
...
#### Select only rows that include "values"/"variables and confidence interval data. ####
...
# for August 2022 iteration no NI council area data as only available for 2020.
...
# Below code is to load 2020 data
...
# filename_NI <- "Labour-market-status-2020_1.XLSX"

# tabname_NI <- "1.1"

# scraped_data_NI <- read_excel(filename_NI, sheet = tabname_NI, skip = 53) %>% 
  # mutate(across(where(is.factor), as.character)) %>% 
  # mutate(across(where(is.character), str_to_sentence)) %>% 
  # mutate(across(where(is.character), str_squish)) 

# first 4 columns are council, value, lower CI, Upper CI
