...
# https://www.ofcom.org.uk/__data/assets/file/0017/224216/202105_fixed_laua_coverage_r01.zip

filename <- "202105_fixed_laua_coverage_r01.csv"
...
scraped_values['Variable Name'] = "Percentage of premises with gigabit-capable broadband (%)"
...
scraped_geographies <- scraped_values %>% 
  mutate(Geography = case_when(
    substr(as.character(laua), 1, 3) == "Ukx" ~ "Country", 
...
scraped_clean <- rename(scraped_geographies, AREACD = laua,
                        AREANM = laua_name,
                        Value = "Gigabit availability (% premises)")
