...
Metric <- "Average travel time in minutes to reach nearest large employment centre (500 + employees), by car"

# Metric_short is the name that the sheet will be in the human readable and jitter input files
Metric_short <- "TIMEtravelWORKCar"
...
tabname <- "2019"
Region_2019 <- read.xlsx(filename, sheet = tabname, rows = c(8, 27:35))
...
Region_2019 <-  rename(Region_2019,  LA_Name = Region,
                       `500EmpCart` = `Places.with.500-4999.jobs`) %>%
  select(all_of(c("LA_Code",
                  "LA_Name",
                  "500EmpCart")))
...
tabname <- "2017"
Region_2017 <- read.xlsx(filename, sheet = tabname, rows = c(8, 27:35))
...
tabname <- "2016"
Region_2016 <- read.xlsx(filename, sheet = tabname, rows = c(8, 27:35))
...
tabname <- "2015"
Region_2015 <- read.xlsx(filename, sheet = tabname, rows = c(8, 27:35))
...
tabname <- "2014"
Region_2014 <- read.xlsx(filename, sheet = tabname, rows = c(8, 27:35))
...
Car_2019['Period']=2019 
Car_2017['Period']=2017 
Car_2016['Period']=2016 
Car_2015['Period']=2015 
Car_2014['Period']=2014 
...
# filter to only include car data

England_car <- filter(England, Mode == "Car")


# Add in necessary columns

England_car['LA_Name']="England"
England_car['LA_Code']="E92000001"
England_car['Year'] = c("2014", "2015", "2016", "2017", "2019")


# Select and rename columns of interest

England_car <-  rename(England_car,  `500EmpCart` = `Places.with.500-4999.jobs`,
                   Period = Year) %>%
  select(all_of(c("LA_Code",
                  "LA_Name",
                  "500EmpCart",
...
scraped_values <- rbind(England_car, Region_2019, Region_2017, Region_2016, Region_2015, Region_2014, 
                        Car_2019, Car_2017, Car_2016, Car_2015, Car_2014)
