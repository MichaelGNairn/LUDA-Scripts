...
Metric <- "Average travel time in minutes to reach nearest large employment centre (500 + employees), by public transport"


# Metric_short is the name that the sheet will be in the human readable and jitter input files
Metric_short <- "TIMEtravelWORKPT"
...
tabname <- "2019"
Region_2019 <- read.xlsx(filename, sheet = tabname, rows = 8:17)
...
Region_2019 <-  rename(Region_2019,  LA_Name = Region,
                       `500EmpPTt` = `Places.with.500-4999.jobs`) %>%
  select(all_of(c("LA_Code",
                  "LA_Name",
                  "500EmpPTt")))
...
tabname <- "2017"
Region_2017 <- read.xlsx(filename, sheet = tabname, rows = 8:17)
...
tabname <- "2016"
Region_2016 <- read.xlsx(filename, sheet = tabname, rows = 8:17)
...
tabname <- "2015"
Region_2015 <- read.xlsx(filename, sheet = tabname, rows = 8:17)
...
tabname <- "2014"
Region_2014 <- read.xlsx(filename, sheet = tabname, rows = 8:17)
...
# removes NAs - (year for cycles and cars)
England_PT <- (na.omit(England))


# Add in necessary columns

England_PT['LA_Name']="England"
England_PT['LA_Code']="E92000001"


# Select and rename columns of interest

England_PT <-  rename(England_PT,  `500EmpPTt` = `Places.with.500-4999.jobs`,
                   Period = Year) %>%
  select(all_of(c("LA_Code",
                  "LA_Name",
                  "500EmpPTt",
...
scraped_values <- rbind(England_PT, Region_2019, Region_2017, Region_2016, Region_2015, Region_2014, 
                        PT_2019, PT_2017, PT_2016, PT_2015, PT_2014)
