...
filename <- "reception obesity.csv"
...
Fingertips_values['Variable Name'] = "Obesity prevalence - children (%)"
...
csv_formatted$Value[csv_formatted$Value == ""] <- "NA"

# remove unwanted geography granulations
csv_formatted <- csv_formatted[!is.na(csv_formatted$Geography),]


# For August 2022 iteration, remove 2021/22 and 2020/21 data as well. 
...
csv_output <- filter(csv_formatted, Period != "202021" & Period != "202122") 
