...
Fingertips_values <- filter(Fingertips_data, Sex == "Persons" & Age == "18+ yrs" & Category == "") 
...
Fingertips_values <- rename(Fingertips_values, `Variable Name` = Indicator.Name)
...
csv_output <- rename(Fingertips_clean, AREACD = Area.Code,
                        Period = Time.period,
